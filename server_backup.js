...
function readJsonl(table) {
  ...
  return fs.readFileSync(fp, "utf8")
    ...
    .filter(Boolean)
    .map(line => {
      try { return JSON.parse(line); } catch { return { parse_error: line }; }
    });
}
...
function requireSession(req, res, next) {
  ...
  if (!participant_id || !session_id) {
    return res.status(400).json({ error: "participant_id and session_id are required" });
  }
  next();
}
...
app.post("/api/login", (req, res) => {
  ...
  const group_condition = req.body?.group_condition || ["control", "ai_assist"][Math.floor(Math.random() * 2)];
  ...
  const participant = {
    ...
    ip_hash_note: "IP is not stored by default in this MVP.",
  ...
  appendJsonl("participants", participant);
  appendJsonl("sessions", session);
...
app.post("/api/event", requireSession, (req, res) => {
  appendJsonl("events", {
    participant_id: req.body.participant_id,
    session_id: req.body.session_id,
    event_type: req.body.event_type || "unknown",
    event_target: req.body.event_target || "",
    event_value: req.body.event_value ?? null,
...
app.post("/api/batch-events", requireSession, (req, res) => {
  ...
  for (const ev of events) {
    appendJsonl("events", {
      ...
      event_value: ev.event_value ?? null,
...
app.post("/api/survey", requireSession, (req, res) => {
  appendJsonl("surveys", {
    participant_id: req.body.participant_id,
    session_id: req.body.session_id,
...
app.post("/api/review", requireSession, (req, res) => {
  const review_text = String(req.body.review_text || "");
  appendJsonl("reviews", {
    participant_id: req.body.participant_id,
    session_id: req.body.session_id,
    review_text,
    word_count: review_text.trim() ? review_text.trim().split(/\s+/).length : 0,
...
app.post("/api/finish", requireSession, (req, res) => {
  appendJsonl("sessions", {
    participant_id: req.body.participant_id,
    session_id: req.body.session_id,
    end_time: nowIso(),
